...
public class GetAndAddAttemptsUntilEndScenario {
    ...
    public static void doScenario(String url, HttpHeaders httpHeaders) {
        ...
        getGameAfterAttempt(url, EXPECTED_SECOND_INPUT, EXPECTED_SECOND_FEEDBACK, 2);
        tryToAddAttempt(url, httpHeaders);
    }

    private final static String EXPECTED_ID = "NOT_ENDED_GAME";
    private final static int EXPECTED_MAX_ATTEMPTS = 2;
            ...
            int attempts)
    {
        ...
        if (attempts == EXPECTED_MAX_ATTEMPTS) {
            assertTrue(responseEntity.getBody().isEnded());
        } else {
            assertFalse(responseEntity.getBody().isEnded());
        }
        assertFalse(responseEntity.getBody().isSolved());
    ...
    private GetAndAddAttemptsUntilEndScenario() {
